note
	...
	purpose: "[
		To facilitate access to data contained in some PS_REPOSITORY.
		]"
	how: "[
...
feature -- Access
	...
	maximum_primary_key (a_database: PS_REPOSITORY; a_tuple_query: PS_TUPLE_QUERY [DATA_IDENTIFIED]): INTEGER_64
		...
		do
			create l_projection.make_from_array (<<{DATA_IDENTIFIED}.primary_key_name>>)
	...
	minimum_primary_key (a_database: PS_REPOSITORY; a_tuple_query: PS_TUPLE_QUERY [DATA_IDENTIFIED]): INTEGER_64
		...
		do
			...
			if not l_tested and Result = 1 then
				Result := {DATA_IDENTIFIED}.no_primary_key
			end
			check zero_primary: l_tested implies Result > {DATA_IDENTIFIED}.no_primary_key end
		end
